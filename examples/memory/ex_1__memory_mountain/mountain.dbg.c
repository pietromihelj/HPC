...
int main( int argc, char **argv )
 {
  ...
  FILE *logfile = fopen( "mountain.log", "w" );
  ...
  if ( logfile != NULL ) {
    fprintf ( logfile, "# Memory mountain (MB/sec)\n"
	     "--\t" );
    for (int stride = 1; stride <= MAXSTRIDE; stride++)
      fprintf ( logfile, "%d\t", stride );
    fprintf ( logfile, "\n" );
  }
  ...
  for (int size = MAXBYTES; size >= MINBYTES; size >>= 1)
    {
      ...
      if ( logfile != NULL ) fprintf ( logfile, "%d\t", log2size_kb );
      ...
      for (int stride = 1; stride <= MAXSTRIDE; stride++)
	{
	  ...
	  if ( logfile != NULL ) fprintf ( logfile, "[ %6.4g - %6.4g]\t", ovrhd*1000, timing*1000  );
	}
