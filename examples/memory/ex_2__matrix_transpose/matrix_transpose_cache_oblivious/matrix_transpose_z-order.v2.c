...
int main ( int argc, char **argv )
{
  ...
  uint32_t padding = (argc > 2 ? atoi(*(argv+2)) : 1 );
  k = ( 1 << k );
  // ······························································
  
  // the matrixm size is k^2
  // however, we add an offset to avoid the
  // cahce's set conflicts due to power-of-two
  // alignment
  // k_pad will be the number_of columns
  uint32_t k_pad    = k + padding;
  ...
  for ( uint32_t r = 0, counter = 0; r < k; r++ )
    {
      uint32_t offset = r*k_pad;
      for ( uint32_t c = 0; c < k_pad; c++, counter++ ) {
	src [ offset + c ] = (dtype)counter;
	dst [ offset + c ] = (dtype)counter; }
    }
  ...
  for ( int i = 0, counter = 0; i < k; i++ )
    {
      int row_offset_src = i*k_pad;
      for ( int j = 0; j < k; j++ )
	{
	  int row_offset_dst = j*k_pad;
